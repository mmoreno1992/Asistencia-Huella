...
class FS28DemoActivity : AppCompatActivity(), KoinComponent {
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBotonTomarAsistencia = findViewById(R.id.botonTomarAsistencia)
        ...
        mBotonTomarAsistencia.setOnClickListener {
            if (codigoEmpleado.text.toString().isBlank())
                compararImagenes()
            else {
        ...
        mButtonSave?.setOnClickListener {
            /* Toast.makeText(this, "el drawable es ${mFingerImage?.drawable == R.drawable.ic_picture}", Toast.LENGTH_LONG)
                 .show()
            */
    ...
    private fun compararImagenesPorCodigo(codigo: Int) {
        lifecycleScope.launch {
            ...
            getExternalFilesDir(Environment.DIRECTORY_PICTURES)?.listFiles { _, name ->
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    if (score >= 40) {
                        Toast.makeText(
                            this@FS28DemoActivity,
                            "Se registró la asistencia del colaborador $codigo",
                            Toast.LENGTH_LONG
                        ).show()
                    } else {
                        Toast.makeText(
                            this@FS28DemoActivity,
                            "No se encontro coincidencia para el colaborador $codigo",
                            Toast.LENGTH_SHORT
                        )
                            .show()
                    }
                ...
                if (!huellaEncontrada) {
                    ...
                    if (score >= 40) {
                        ...
                        CoroutineScope(Dispatchers.Main).launch {
                            ...
                            Toast.makeText(
                                this@FS28DemoActivity,
                                "Se registró asistencia para el colaborador ${it.name.split("_")[0]}",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                        ...
                        CoroutineScope(Dispatchers.Main).launch {
                            mMessage?.text = "Archivo ${it.name} NO COINCIDE con la huella."
                        }
                    }
    ...
    private fun saveImage() {
        ...
        if (EasyPermissions.hasPermissions(this, *perms)) {
            ...
            val fileFormat = ".bmp"
            val fileName = "$dir/${codigoEmpleado.text.toString()}_${Date()}"
...
/*
            ...
            serverIntent.putExtra("CODIGO_EMPLEADO", codigoEmpleado.text.toString())
