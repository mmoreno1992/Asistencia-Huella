...
import santaana.asistencia.ftrwsqandroidhelper;
...
import static santaana.asistencia.ConstantsKt.MESSAGE_SHOW_IMAGE;
import static santaana.asistencia.ConstantsKt.MESSAGE_SHOW_MSG;
import static santaana.asistencia.ConstantsKt.MESSAGE_SHOW_PROGRESSBAR;
...
public class RegistrarHuellaBluetoothDataService {
   ...
   */
  public RegistrarHuellaBluetoothDataService(Handler handler) {
  ...
  private class AcceptThread extends Thread {
    ...
    public void run() {
      ...
      while (mState != STATE_CONNECTED) {
        if (RegistrarHuellaActivity.stopped)
          break;
        try {
        ...
        if (socket != null) {
          synchronized (RegistrarHuellaBluetoothDataService.this) {
            switch (mState) {
              case STATE_LISTEN:
  ...
  private class ConnectedThread extends Thread {
    ...
    public ConnectedThread(BluetoothSocket socket) {
      ...
      RegistrarHuellaActivity.connected = true;
    }
    ...
    public void run() {
      ...
      while (true) {
        ...
        while (mbytesTotal > 0) {
          ...
          if (RegistrarHuellaActivity.stopped || (mState != STATE_CONNECTED))
            break;
          try {
          ...
          if (mbytesRead > 0) {
            ...
            if (mCommandLength < 13) {
              if (mbytesRead > (13 - mCommandLength)) {
                ...
                System.arraycopy(buffer, 13 - mCommandLength, RegistrarHuellaActivity.mImageFP, mbytesTotalRead, mbytesRead - (13 - mCommandLength));
              ...
              if (mCommandLength == 13) {
                ...
                if (!ValidateCommandData(mCommand, Size)) {
                  mCommandLength = 0;
                  mbytesTotalRead = 0;
                  mHandler.obtainMessage(MESSAGE_DATA_ERROR, mErrorCode, -1).sendToTarget();
                  error = true;
                  ...
                  if (mCommand[1] == 0x36) // start convert WSQ
                  {
                    ...
                    mCommandLength = 0;
                    mbytesTotalRead = 0;
                    mHandler.obtainMessage(MESSAGE_SHOW_MSG, -1, -1, "Got convert WSQ cmd 0x36").sendToTarget();
                  ...
                  } else if (mCommand[1] == 0x0F) //download WSQ
                  {
                    ...
                    mHandler.obtainMessage(MESSAGE_SHOW_MSG, -1, -1, strTimeMsg).sendToTarget();
                    ResponseToFS28((byte) 0x00, (byte) 0x40);
                  ...
                  } else if (mCommand[1] == 0x4D) //0x4D command means FS28 start to download sample, the sample size is always 666 bytes
                  {
                    ...
                    mCommandLength = 0;
                    mbytesTotalRead = 0;
                    mHandler.obtainMessage(MESSAGE_DATA_ERROR, ERROR_UNKNOWN_COMMAND, -1).sendToTarget();
                    error = true;
                    ResponseToFS28((byte) 0x00, (byte) 0x41);
                    break;
                  }
                  if (mbytesTotal > 153602) {
                    ...
                    mCommandLength = 0;
                    mbytesTotalRead = 0;
                    mHandler.obtainMessage(MESSAGE_DATA_ERROR, ERROR_IMAGE_SIZE_TOO_LARGE, -1).sendToTarget();
                    error = true;
                    ResponseToFS28((byte) 0x00, (byte) 0x41);
                    break;
                  }
            ...
            {
              System.arraycopy(buffer, 0, RegistrarHuellaActivity.mImageFP, mbytesTotalRead, mbytesRead);
              mbytesTotal -= mbytesRead;
              mbytesTotalRead += mbytesRead;
              mnCurrentStep = mbytesTotalRead * 100 / (mTotalImageSize + 2);
              if (mnCurrentStep > RegistrarHuellaActivity.mStep) {
                RegistrarHuellaActivity.mStep = mnCurrentStep;
                mHandler.obtainMessage(MESSAGE_SHOW_PROGRESSBAR).sendToTarget();
              }
              StartTimer(TIMEOUT_2);    // set timeout 3sec
            }
          }// if( mbytesRead>0 )
        ...
        if (!error) {
          RegistrarHuellaActivity.mReceivedDataType = mDataType;
          ResponseToFS28((byte) 0x00, (byte) 0x40);
          mTimeDownloadImage = SystemClock.uptimeMillis() - mTimeDownloadImage;
          if (mDataType == DATA_TYPE_WSQIMAGE) {
            RegistrarHuellaActivity.mWsqImageFP = new byte[mTotalImageSize];
            System.arraycopy(RegistrarHuellaActivity.mImageFP, 0, RegistrarHuellaActivity.mWsqImageFP, 0, mTotalImageSize);
            mHandler.obtainMessage(MESSAGE_SHOW_MSG, -1, -1, "Convert WSQ to RAW...").sendToTarget();
            mTimeConvertWSQInAndroid = SystemClock.uptimeMillis();
            ftrwsqandroidhelper helper = new ftrwsqandroidhelper();
            int sizeRaw = helper.GetWsqImageRawSize(RegistrarHuellaActivity.mWsqImageFP);
            if (sizeRaw > 0) {
              byte[] rawImg = new byte[sizeRaw];
              if (helper.ConvertWsqToRaw(RegistrarHuellaActivity.mWsqImageFP, rawImg)) {
                System.arraycopy(rawImg, 0, RegistrarHuellaActivity.mImageFP, 0, sizeRaw);
            ...
            ConvertToHostSample(RegistrarHuellaActivity.mImageFP);
            strTimeMsg = String.format("Futronic Sample TDn:%d(ms)", mTimeDownloadImage);
          } else if (mDataType == DATA_TYPE_ANSI_SAMPLE) {
            RegistrarHuellaActivity.mANSISample = new byte[mTotalImageSize];
            System.arraycopy(RegistrarHuellaActivity.mImageFP, 0, RegistrarHuellaActivity.mANSISample, 0, mTotalImageSize);
            strTimeMsg = String.format("ANSI Sample TDn:%d(ms)", mTimeDownloadImage);
          } else if (mDataType == DATA_TYPE_ISO_SAMPLE) {
            RegistrarHuellaActivity.mISOSample = new byte[mTotalImageSize];
            System.arraycopy(RegistrarHuellaActivity.mImageFP, 0, RegistrarHuellaActivity.mISOSample, 0, mTotalImageSize);
        ...
        ResetRecvVariables();
        RegistrarHuellaActivity.mStep = 0;
      } // while (true)
    }

    public void ConvertToHostSample(byte[] FamSample) {
      RegistrarHuellaActivity.mHostSample[0] = (byte) 0x9d;
      RegistrarHuellaActivity.mHostSample[1] = 0x02;
      RegistrarHuellaActivity.mHostSample[2] = 0x02;
      RegistrarHuellaActivity.mHostSample[3] = 0x02;
      RegistrarHuellaActivity.mHostSample[4] = 0x00;
      System.arraycopy(FamSample, 0, RegistrarHuellaActivity.mHostSample, 5, 664);
    }
