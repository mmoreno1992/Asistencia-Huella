...
public class TomarAsistenciaBluetoothDataService {
   ...
   */
  public TomarAsistenciaBluetoothDataService(Handler handler) {
  ...
  private class AcceptThread extends Thread {
    ...
    public void run() {
      ...
      while (mState != STATE_CONNECTED) {
        if (TomarAsistenciaActivity.stopped)
          break;
        try {
        ...
        if (socket != null) {
          synchronized (TomarAsistenciaBluetoothDataService.this) {
            switch (mState) {
              case STATE_LISTEN:
  ...
  private class ConnectedThread extends Thread {
    ...
    public ConnectedThread(BluetoothSocket socket) {
      ...
      TomarAsistenciaActivity.connected = true;
    }
    ...
    public void run() {
      ...
      while (true) {
        ...
        while (mbytesTotal > 0) {
          ...
          if (TomarAsistenciaActivity.stopped || (mState != STATE_CONNECTED))
            break;
          try {
          ...
          if (mbytesRead > 0) {
            ...
            if (mCommandLength < 13) {
              if (mbytesRead > (13 - mCommandLength)) {
                ...
                System.arraycopy(buffer, 13 - mCommandLength, TomarAsistenciaActivity.mImageFP, mbytesTotalRead, mbytesRead - (13 - mCommandLength));
            ...
            {
              System.arraycopy(buffer, 0, TomarAsistenciaActivity.mImageFP, mbytesTotalRead, mbytesRead);
              mbytesTotal -= mbytesRead;
              mbytesTotalRead += mbytesRead;
              mnCurrentStep = mbytesTotalRead * 100 / (mTotalImageSize + 2);
              if (mnCurrentStep > TomarAsistenciaActivity.mStep) {
                TomarAsistenciaActivity.mStep = mnCurrentStep;
        ...
        if (!error) {
          TomarAsistenciaActivity.mReceivedDataType = mDataType;
          ResponseToFS28((byte) 0x00, (byte) 0x40);
          mTimeDownloadImage = SystemClock.uptimeMillis() - mTimeDownloadImage;
          if (mDataType == DATA_TYPE_WSQIMAGE) {
            TomarAsistenciaActivity.mWsqImageFP = new byte[mTotalImageSize];
            System.arraycopy(TomarAsistenciaActivity.mImageFP, 0, TomarAsistenciaActivity.mWsqImageFP, 0, mTotalImageSize);
            ...
            int sizeRaw = helper.GetWsqImageRawSize(TomarAsistenciaActivity.mWsqImageFP);
            if (sizeRaw > 0) {
              byte[] rawImg = new byte[sizeRaw];
              if (helper.ConvertWsqToRaw(TomarAsistenciaActivity.mWsqImageFP, rawImg)) {
                System.arraycopy(rawImg, 0, TomarAsistenciaActivity.mImageFP, 0, sizeRaw);
            ...
            ConvertToHostSample(TomarAsistenciaActivity.mImageFP);
            strTimeMsg = String.format("Futronic Sample TDn:%d(ms)", mTimeDownloadImage);
          } else if (mDataType == DATA_TYPE_ANSI_SAMPLE) {
            TomarAsistenciaActivity.mANSISample = new byte[mTotalImageSize];
            System.arraycopy(TomarAsistenciaActivity.mImageFP, 0, TomarAsistenciaActivity.mANSISample, 0, mTotalImageSize);
            strTimeMsg = String.format("ANSI Sample TDn:%d(ms)", mTimeDownloadImage);
          } else if (mDataType == DATA_TYPE_ISO_SAMPLE) {
            TomarAsistenciaActivity.mISOSample = new byte[mTotalImageSize];
            System.arraycopy(TomarAsistenciaActivity.mImageFP, 0, TomarAsistenciaActivity.mISOSample, 0, mTotalImageSize);
        ...
        ResetRecvVariables();
        TomarAsistenciaActivity.mStep = 0;
      } // while (true)
    }

    public void ConvertToHostSample(byte[] FamSample) {
      TomarAsistenciaActivity.mHostSample[0] = (byte) 0x9d;
      TomarAsistenciaActivity.mHostSample[1] = 0x02;
      TomarAsistenciaActivity.mHostSample[2] = 0x02;
      TomarAsistenciaActivity.mHostSample[3] = 0x02;
      TomarAsistenciaActivity.mHostSample[4] = 0x00;
      System.arraycopy(FamSample, 0, TomarAsistenciaActivity.mHostSample, 5, 664);
    }
